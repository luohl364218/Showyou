...
import com.zju.campustour.presenter.chatting.tools.HanyuPinyin;
import com.zju.campustour.view.chatting.adapter.AllMembersAdapter;
...
public class MembersInChatActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mIsDeleteMode = getIntent().getBooleanExtra(Constants.DELETE_MODE, false);
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(Constants.RESULT_CODE_ALL_MEMBER, intent);
                    finish();
                    break;
                case R.id.right_btn:
                    if (mIsDeleteMode) {
        ...
        JMessageClient.addGroupMembers(mGroupId, list, new BasicCallback() {
            ...
            public void gotResult(final int status, final String desc) {
                ...
                if (status == 0) {
                    // 添加群成员
                    refreshMemberList();
                    showToast(mContext.getString(R.string.added));
                } else {
                    HandleResponseCode.onHandle(mContext, status, true);
                }
            }
